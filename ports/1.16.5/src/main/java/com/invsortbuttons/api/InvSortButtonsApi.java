...
/**
 * Public entry point for mods integrating with PlayerInventory Sorter Buttons.
 *
...
public final class InvSortButtonsApi {
    ...
    public static void sortOpenContainer(int mode) {
        NetworkHandler.CHANNEL.sendToServer(new SortPacket(mode, false));
    }

    /** Sort the player's main inventory (hotbar untouched). Client side only. */
    public static void sortPlayerInventory() {
        NetworkHandler.CHANNEL.sendToServer(new SortPacket(SORT_DEFAULT, true, false));
    }
}
