...
public class MoveAllIconButton extends MiniIconButton {
    ...
    public void onPress(net.minecraft.client.input.InputWithModifiers input) {
        ...
        if (this.intoContainer) {
            ...
            boolean includeHotbar = input.hasShiftDown();
            for (Slot s : screen.getMenu().slots) {
                ...
                mc.gameMode.handleContainerInput(containerId, s.index, 0, ContainerInput.QUICK_MOVE, mc.player);
            }
        } else {
            ...
            for (Slot cs : chest) {
                ...
                mc.gameMode.handleContainerInput(containerId, cs.index, 0, ContainerInput.PICKUP, mc.player);
                for (int pass = 0; pass < 2 && !screen.getMenu().getCarried().isEmpty(); pass++) {
                    for (Slot t : targets) {
                        ...
                        boolean fits = pass == 0
                                ...
                                : !t.hasItem();
                        if (fits) {
                            mc.gameMode.handleContainerInput(containerId, t.index, 0, ContainerInput.PICKUP, mc.player);
                        }
                    }
                }
