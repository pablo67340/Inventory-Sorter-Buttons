...
/**
 * Implement this on your {@code Container} to control which slots the sorter
 * treats as the "chest section". Useful when auto-detection gets it wrong:
 * menus with ghost/filter slots, mixed storage + machine slots, etc.
 *
 * <p>Checked on both sides — the client uses it for button placement, the
 * server for the actual sort — so implement it on the shared container class.
 */
