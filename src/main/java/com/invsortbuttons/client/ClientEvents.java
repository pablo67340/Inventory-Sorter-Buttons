...
public final class ClientEvents {
    ...
    public static void onScreenRender(ScreenEvent.Render.Post event) {
        for (GuiEventListener child : event.getScreen().children()) {
            if (child instanceof MiniIconButton button && button.isHoveredOrFocused()) {
                event.getScreen().renderTooltip(event.getPoseStack(), button.getTooltipText(),
    ...
    public static void onMouseClick(ScreenEvent.MouseButtonPressed.Pre event) {
        ...
        if (event.getButton() == GLFW.GLFW_MOUSE_BUTTON_LEFT && ISBConfig.SHORTCUTS.get()) {
            ...
            if (net.minecraft.client.gui.screens.Screen.hasAltDown() && hovered.hasItem()) {
                mc.gameMode.handleInventoryMouseClick(containerId,
                        hovered.index, 1, ClickType.THROW, mc.player);
                event.setCanceled(true);
            } else if (InputConstants.isKeyDown(window, GLFW.GLFW_KEY_SPACE)) {
                ...
                for (Slot s : screen.getMenu().slots) {
                    if (s.hasItem() && SortEngine.isPlayerSlot(s, mc.player.getInventory()) == fromPlayer
                            && ItemStack.isSameItemSameTags(s.getItem(), reference)) {
                        mc.gameMode.handleInventoryMouseClick(containerId,
                                s.index, 0, ClickType.QUICK_MOVE, mc.player);
                    }
                }
                ...
                for (Slot s : screen.getMenu().slots) {
                    ...
                    boolean sameWithRoom = s.hasItem()
                            && ItemStack.isSameItemSameTags(s.getItem(), hovered.getItem())
